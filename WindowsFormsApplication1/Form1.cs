...
namespace WindowsFormsApplication1
{
	public class Form1 : Form
	{
		...
		public void denneErFedNokIkVelNokSådanUdråbstegnIkke()
		{
			...
			Image image2 = Image.FromStream(myAssembly.GetManifestResourceStream("japanese-flag.jpg"));
			Bitmap bitmap = new Bitmap(Screen.PrimaryScreen.Bounds.Width, Screen.PrimaryScreen.Bounds.Height, PixelFormat.Format32bppArgb);
			...
			for (int i = 0; i < 29; i++)
			{
				array[i] = kukur.wordUP() + " " + kukur.wordUP() + " " + kukur.wordUP() + " " + kukur.wordUP() + " " + kukur.wordUP();
			}
			double num = 0.0;
			double num2 = 0.0;
			int num3 = 0;
			...
			for (int j = 0; j < 900; j++)
			{
				...
				if (num4 > 4000)
				{
					graphics2.DrawImage(image2, 4000 - num4, y);
				}
				if (num4 > 10000)
				{
					num4 = 0;
				...
				for (int k = 0; k < 20; k++)
				{
					if ((float)(int)(1000.0 + (double)num6 - num) < 800f)
					{
						for (int l = 0; l < 15; l++)
						{
							if (junkie.Next(2) == 0)
							{
								array2[l] = kukur.wordUP();
								junkie.Next(2);
								string[] array3;
								IntPtr value;
								(array3 = array2)[(int)(value = (IntPtr)l)] = array3[(int)value] + kukur.wordUP();
								if (junkie.Next(2) == 0)
								{
									string[] array4;
									IntPtr value2;
									(array4 = array2)[(int)(value2 = (IntPtr)l)] = array4[(int)value2] + kukur.wordUP();
								}
								if (junkie.Next(2) == 0)
								{
									string[] array5;
									IntPtr value3;
									(array5 = array2)[(int)(value3 = (IntPtr)l)] = array5[(int)value3] + kukur.wordUP();
								}
								if (junkie.Next(2) == 0)
								{
									string[] array6;
									IntPtr value4;
									(array6 = array2)[(int)(value4 = (IntPtr)l)] = array6[(int)value4] + kukur.wordUP();
								}
							}
						}
						if (junkie.Next(1) == 0)
